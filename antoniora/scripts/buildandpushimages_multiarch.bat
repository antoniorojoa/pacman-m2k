...
:DEFAULT_REGISTRY
    ...
    SET REGISTRY_NAMESPACE=antoniora

:MAIN
:: Uncomment the below line if you want to enable login before pushing
:: docker login %REGISTRY_URL%

echo "building and pushing image pacman"
pushd source\pacman-master\pacman-master
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/pacman .
popd
